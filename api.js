  ...
  return withCors(ok({ user_id: id }));
      ...
      if (path === "/api/listings" && req.method === "POST") {
        ...
        if (kind === "ac") {
          ...
          // ⚠️ MVP: đang lưu plaintext. Sau mình đổi sang AES-GCM dùng AES_SECRET.
          await env.DB.prepare(
      ...
      if (path === "/api/orders" && req.method === "POST") {
        ...
        try {
          ...
          await env.DB.prepare("UPDATE listings SET quantity=?, status=?, updated_at=? WHERE id=?")
            .bind(newQty, newStatus, now, listing.id).run();
          ...
          await env.DB.prepare(
            ...
            orderId, u.userId, listing.seller_id, listing.id,
            listing.price_cents, qty, subtotal, platformFee, sellerIncome, now
          ).run();
          ...
          ).bind(crypto.randomUUID(), u.userId, -subtotal, orderId, `Buy listing ${listing.id}`, now).run();

          await env.DB.prepare(
            `INSERT INTO wallet_ledger (id, user_id, type, amount_cents, ref_type, ref_id, note, created_at)
             VALUES (?, ?, 'sale_income', ?, 'order', ?, ?, ?)`
          ).bind(crypto.randomUUID(), listing.seller_id, sellerIncome, orderId, `Sold listing ${listing.id}`, now).run();
      ...
      if (path.startsWith("/api/orders/") && path.endsWith("/secret") && req.method === "GET") {
        ...
        const listing = await env.DB.prepare("SELECT kind FROM listings WHERE id=?")
      ...
      if (path.startsWith("/api/orders/") && path.endsWith("/feedback") && req.method === "POST") {
        ...
        if (type === "trust") {
          await env.DB.prepare("UPDATE users SET reputation = reputation + 1, updated_at=? WHERE id=?")
            .bind(Date.now(), order.seller_id).run();
        } else {
      ...
      if (path.startsWith("/api/admin/disputes/") && path.endsWith("/decision") && req.method === "POST") {
        ...
        try {
          ...
          ).bind(crypto.randomUUID(), order.buyer_id, order.subtotal_cents, disputeId, `Refund order ${order.id}`, now).run();
          ...
          ).bind(crypto.randomUUID(), order.seller_id, -order.seller_income_cents, disputeId, `Chargeback order ${order.id}`, now).run();
...
async function getUserFromSession(req, env) {
  ...
  env.DB.prepare("UPDATE sessions SET last_seen_at=? WHERE token_hash=?")
    .bind(Date.now(), tokenHash).run().catch(() => {});
...
async function computeQuota(env, userId) {
  ...
  ).bind(u.total_deposit_cents).first();
